...
public class DOMContext {
  ...
  public void waitForPageToLoad() {
    ...
    synchronized (lock) {
      newPageLoaded = false;
      try {
        currentDocument = getCurrentDocumentAndCheckTheRemoteObject();
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
  ...
  private Node getCurrentDocumentAndCheckTheRemoteObject() {
    Node res = null;
    while (true) {
      try {
        res = inspector.getDocument();
        res.getRemoteObject();
        break;
      } catch (Exception e) {
        System.err.println("Workaround in DOMContext, the given document is corrupted, nodeId "+res.getNodeId());
        // ignore.
      }
    }
    return res;
